...
class SerapiInstance(threading.Thread):
    ...
    # Get the AST of the message at `self.cur_state`. That should be
    # the AST of the argument to the last 'run_stmt'.
    def get_AST(self) -> Any:
        assert self.message_queue.empty()
        self._send_acked(f"(Query ((sid {self.cur_state})) Ast)")
        msg = normalizeMessage(self._get_message(), depth=500)
        self._get_completed()
        assert self.message_queue.empty()

        match msg:
            case ['Answer', int, ['ObjList', [['CoqAst', result]]]]:
                return ['CoqAst', result]
            case _: assert False
    ...
    def run_stmt(self, stmt: str, timeout: Optional[int] = None):
        ...
        # sometimes errors are expected.
            ...
            print(f"[coq-serapy] exception in run-stmt, stmt='{stmt}'")
    ...
    def _send_flush(self, cmd: str):
        ...
        if self.log_outgoing_messages:
            with open(self.log_outgoing_messages, 'wa') as f:
                print(cmd, file=f)
        try:
    ...
    def _get_message(self, complete=False) -> Any:
        ...
        try:
            res = loads(msg_text, nil=None)
            # print(f"    [get_message] MSG = {res}")
            return res
