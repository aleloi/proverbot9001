...
#CMD_FILE = open('/home/alex/proverbot9001/subproc_commands', 'w')

def srun(*args, **kwargs):
    #print(f"args: {args}, kwargs: {kwargs}", file=CMD_FILE)
    #CMD_FILE.flush()
...
class SerapiInstance(threading.Thread):
    ...
    def __init__(self, coq_command: List[str], module_name: Optional[str],
        ...
        eprint(f"[aleloi-serapy]: SerapiInstance.__init__: coq command is: {coq_command}")
        try:
        ...
        if switch is not None:
            ...
            self.setup_opam_env()
        self.version_string = srun(["sertop", "--version"], stdout=subprocess.PIPE,
                                             text=True).stdout
        print(f"[aleloi-serapy]: coq minor version is: {self.coq_minor_version()}")
        ...
        # Clear the file if it has content already.
        if self.log_outgoing_messages:
            open(self.log_outgoing_messages, 'w').close()
        ...
        if self.use_hammer:
            try:
                ...
                raise
        eprint(f"[aleloi-serapy]: serapy c-tor is done.")
    ...
    def run_stmt(self, stmt: str, timeout: Optional[int] = None):
        ...
        try:
            ...
            for stm in preprocess_command(stmt):
                ...
                what_to_send = "(Add () \"{}\")\n".format(stm)
                eprint(f"[aleloi-serapy]: serapy.run_stmt; sending {what_to_send}", guard=self.verbose > 0)
                self._send_acked(what_to_send)
    ...
    def run_into_next_proof(self, commands: List[str]) \
        ...
        eprint(f"[aleloi-serapy] serapy.run_into_next_proof, there are {len(commands)} commands starting with {commands[:1]}", guard=self.verbose>0)
    ...
    def set_switch(self, switch: str) -> None:
        # assert not self.init_is_done, "only in c-tor!"
        if not self.init_is_done:
            eprint(f"[aleloi coq-serapy]: switching to {switch} not from c-tor!")
        env_string = srun(f"opam env --switch={switch} --set-switch",
    ...
    def setup_opam_env(self) -> None:
        env_string = srun(f"opam env", shell=True, stdout=subprocess.PIPE,
                                    text=True).stdout
        self._setup_opam_env_from_str(env_string)
    ...
    def _send_flush(self, cmd: str):
        ...
        if self.log_outgoing_messages:
            with open(self.log_outgoing_messages, 'a') as f:
                print(cmd, file=f)
                f.flush()
        try:
